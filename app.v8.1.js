...
function regionShapes(){
  // Asymmetric wedges: slimmer on the left (theta ≈ π), wider on the right (theta ≈ 0)
  // r0/r1 are in world units; convert to screen via state.scale later.
  const rings = [
...
function drawRegion(shape){
  ...
  // angular coverage: leave a gap on the far left to match the poster's void/Unknown
  const theta0 = -Math.PI*0.90, theta1 = Math.PI*0.90;
  const steps = 96;

  // radius modulation to slim the left side (theta≈π)
...
function drawRegionLabels(){
  ...
  const theta = 0; // label along the east side
  rings.forEach(ring=>{
    const color = REG_COL[ring.name] || '#cfe3ff';
    const rMid = ((ring.r0+ring.r1)/2) * (0.86) * state.scale;
    const x = tx(Math.cos(theta)*((ring.r0+ring.r1)/2)), y = ty(Math.sin(theta)*((ring.r0+ring.r1)/2));
    // draw text in screen space near x,y but slightly offset to avoid strokes
